...
class SC_Util_PG_MULPAY {
    ...
    function getJobCds($pay_id = MDL_PG_MULPAY_PAYID_CREDIT) {
        ...
        if ($pay_id != MDL_PG_MULPAY_PAYID_CREDIT &&
            ...
            $pay_id != MDL_PG_MULPAY_PAYID_TOKEN) { // クレジットカード以外
...
//            CONVENI_SUNKUS => 'サンクス',
    ...
    function getPayTypeFromPayId($paytype) {
        switch ($paytype) {
            ...
            case MDL_PG_MULPAY_PAYID_TOKEN:
            default:
    ...
    /**
     * クレジットの設定値を取得する
     * 取得順デフォルトは、トークン決済、通常決済の順
     * 該当する決済方法が取得できた時点で検索を終了しその決済方法の設定を採用
     */
    function getCreditConfig($payment_code_list =
                             array(MDL_PG_MULPAY_PAYID_TOKEN,
                                   MDL_PG_MULPAY_PAYID_CREDIT)) {
        $objMdl =& SC_Mdl_PG_MULPAY::getInstance();
        $objQuery =& SC_Query::getSingletonInstance();

        $payment = array();
        foreach ($payment_code_list as $value) {
            $arrVal = array($objMdl->getCode(true), $value);
            $payment = $objQuery->getRow("payment_id", "dtb_payment",
                                         "module_code = ? AND " .
                                         MDL_PG_MULPAY_PAYMENT_COL_PAYID .
                                         " = ? AND del_flg = 0", $arrVal);
            if (!empty($payment)) {
                break;
            }
        }
        ...
        return SC_Util_PG_MULPAY_Ex::getPaymentTypeConfig($payment_id);
    }
    ...
    function isTokenCardPaymentEnable() {
        $objMdl =& SC_Mdl_PG_MULPAY::getInstance();
        $objQuery =& SC_Query::getSingletonInstance();
        $arrVal = array($objMdl->getCode(true), (string)MDL_PG_MULPAY_PAYID_TOKEN);
        if ($objQuery->count("dtb_payment", "module_code = ? AND " . MDL_PG_MULPAY_PAYMENT_COL_PAYID . " = ? AND del_flg = 0", $arrVal)) {
            return true;
        }
        return false;
    ...
    function getJsUrlPath($server_url) {
        $url = MDL_PG_MULPAY_JSSERVER_URL_TEST;

        $objMdl =& SC_Mdl_PG_MULPAY_Ex::getInstance();
        $arrSetting = $objMdl->getUserSettings();

        if (!empty($arrSetting) &&
            isset($arrSetting['connect_server_type']) &&
            $arrSetting['connect_server_type'] == '2') {        // 本番環境
            $url = MDL_PG_MULPAY_JSSERVER_URL_PROD;
        }
...
//                    MDL_PG_MULPAY_PAYID_CREDIT_SAUTH => MDL_PG_MULPAY_PAYNAME_CREDIT_SAUTH,
                    MDL_PG_MULPAY_PAYID_TOKEN => MDL_PG_MULPAY_PAYNAME_TOKEN,
    ...
    function getPaymentTypeCodes() {
        return array(
                    ...
                    MDL_PG_MULPAY_PAYID_TOKEN => MDL_PG_MULPAY_PAYCODE_TOKEN,
