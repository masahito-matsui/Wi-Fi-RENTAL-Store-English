...
require_once(MDL_PG_MULPAY_CLASSEX_PATH . 'client_extends/SC_Mdl_PG_MULPAY_Client_IDnet_Ex.php');

/**
 * 決済モジュール 決済画面ヘルパー：iD決済
 */
class LC_PageHelper_Mdl_PG_MULPAY_IDnet extends LC_PageHelper_Mdl_PG_MULPAY_Base_Ex {
    ...
    function initParam(&$objFormParam, &$arrPaymentInfo, &$arrOrder) {
        $objFormParam->addParam("iD決済端末メールアドレス", "MailAddress", 128, 'n', array("EXIST_CHECK", "MAX_LENGTH_CHECK", "MOBILE_EMAIL_CHECK"), $arrOrder['order_email']);
    }
    ...
    function modeAction($mode, &$objFormParam, &$arrOrder, &$objPage) {
        ...
        case 'next':
            ...
            if (SC_Utils_Ex::isBlank($objPage->arrErr)) {
                // 決済実行
                $objClient = new SC_Mdl_PG_MULPAY_Client_IDnet_Ex();
                ...
                if ($result) {
                    ...
                    if (SC_Display_Ex::detectDevice() === DEVICE_TYPE_PC) {
                        ...
                        $arrPayData = SC_Util_PG_MULPAY_Ex::getOrderPayData($arrOrder['order_id']);
                        $objPage->AccessID = $arrPayData['AccessID'];
                        $objPage->tpl_url = $objClient->getIdStartPageUrl();
    ...
    function getFormBloc() {
        ...
        $bloc_id =  $arrBlocId['pg_mulpay_idnet'][ $device_type_id ];
        if ($bloc_id) {
