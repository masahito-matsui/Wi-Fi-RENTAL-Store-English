...
require_once(MDL_PG_MULPAY_CLASSEX_PATH . 'client_extends/SC_Mdl_PG_MULPAY_Client_Docomo_Ex.php');

/**
 * 決済モジュール 決済画面ヘルパー：ドコモケータイ払い
 */
class LC_PageHelper_Mdl_PG_MULPAY_Docomo extends LC_PageHelper_Mdl_PG_MULPAY_Base_Ex {
    ...
    function modeAction($mode, &$objFormParam, &$arrOrder, &$objPage) {
        ...
        case 'next':
            ...
            if (SC_Utils_Ex::isBlank($objPage->arrErr)) {
                // 決済実行
                $objClient = new SC_Mdl_PG_MULPAY_Client_Docomo_Ex();
    ...
    function getFormBloc() {
        ...
        $bloc_id =  $arrBlocId['pg_mulpay_docomo'][ $device_type_id ];
        if ($bloc_id) {
