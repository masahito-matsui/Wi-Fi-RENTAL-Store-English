 ...
 * 決済モジュール 決済処理: au 継続
 */
class SC_Mdl_PG_MULPAY_Client_AuContinuance extends SC_Mdl_PG_MULPAY_Client_Base_Ex {
    ...
    function doAuReturn($arrOrder, $arrParam, $arrPaymentInfo) {
        ...
        if ($arrParam['Status'] != 'REGISTER') {
            $msg = 'auかんたん決済に失敗しました。(ST=' . htmlspecialchars($arrParam['Status']) . ')';
        }

        if (SC_Utils_Ex::isBlank($arrParam['AuContinuAccountId'])) {
            $msg = 'auかんたん決済がキャンセルされました。';
            $this->setError($msg);
        }
    ...
    function doPaymentRequest($arrOrder, $arrParam, $arrPaymentInfo) {
        ...
        $server_url = $arrMdlSetting['server_url'] . 'EntryTranAuContinuance.idPass';
...
//            'FirstTax',
        ...
        $server_url = $arrMdlSetting['server_url'] . 'ExecTranAuContinuance.idPass';
        $arrSendKey = array(
            'ShopID',
            'ShopPass',
            'AccessID',
