...
class LC_Page_Shopping extends LC_Page_Ex
{
    ...
    public function action()
    {
        ...
        $nonmember_title = 'Customer Information';

	//1080円の追加送料の商品ID（実際の値はdtb_products_class.product_class_id）と個数を追加
	$cartItems = $objCartSess->getAllCartList();
	$arrCartKeys = $objCartSess->getKeys();
  //物理SIMはこの機能を使わない
  $bsim_flg = 0;
//echo "<pre>"; print_r($cartItems); print_r($arrCartKeys); echo "</pre>"; exit;
	foreach ($arrCartKeys as $cart_key) {
		foreach($cartItems[$cart_key] as $key => $value){
      if($value["productsClass"]["maker_id"] == 4) {
        $bsim_flg = 1;
      }
			if($value["id"] == 228) {
				$objCartSess->delProduct($value["cart_no"], $cart_key);
			}
		}
	}
	...
	if(($_SESSION['pre_deliv_id'] == 3) || ($_SESSION['pre_deliv_id'] == 1) || ($_SESSION['pre_deliv_id'] == 9)) {
		if($quantity > 2) {
			if($add_flg == 0) {
				//追加
				$status_tmp = 1;
        $add_count = $quantity - 2;
				$objCartSess->addProduct(228, $add_count);
			}
		} else {
			if($add_flg == 1) {
				//削除
	...
	//配送都道府県をセッションに保存
	if($_REQUEST['shipping_pref'] > 0) {
		$_SESSION['pre_shipping_pref'] = $_REQUEST['shipping_pref'];
	}

	//選択した空港をセッションに保存
	if($_REQUEST['shipping_airport'] > 0) {
		$_SESSION['shipping_airport'] = $_REQUEST['shipping_airport'];
	}
...
$this->lfInitLoginFormParam($objFormParam);
		...
		if($_REQUEST['order_country_id'] > 0) {
			$_SESSION['order_country_id'] = $_REQUEST['order_country_id'];
		}
    ...
    {
        $objFormParam->addParam('Memory', 'login_memory', INT_LEN, 'n', array('MAX_LENGTH_CHECK', 'NUM_CHECK'));
        $objFormParam->addParam('E-mail', 'login_email', '' , 'a', array('EXIST_CHECK', 'EMAIL_CHECK', 'SPTAB_CHECK' ,'EMAIL_CHAR_CHECK'));
        $objFormParam->addParam('Password', 'login_pass', PASSWORD_MAX_LEN, '', array('EXIST_CHECK', 'MAX_LENGTH_CHECK', 'SPTAB_CHECK'));
    }
...
//                return 'deliv.php';
    ...
    public function lfCheckError(&$objFormParam)
    {
        ...
        $objErr->doFunc(array('E-mail', 'E-mail（Confirm）', 'order_email', 'order_email02'), array('EQUAL_CHECK'));
    ...
    public function lfGetErrorMessage($error)
    {
        switch ($error) {
            ...
            default:
                $msg = 'Error: Your email address or password is invalid.';
        }
