...
require_once(MDL_PG_MULPAY_CLASSEX_PATH . 'client_extends/SC_Mdl_PG_MULPAY_Client_CVS_Ex.php');

/**
 * 決済モジュール 決済画面ヘルパー：コンビニ決済
 */
class LC_PageHelper_Mdl_PG_MULPAY_CVS extends LC_PageHelper_Mdl_PG_MULPAY_Base_Ex {
    ...
    function initParam(&$objFormParam, &$arrPaymentInfo, &$arrOrder) {
        $objFormParam->addParam("コンビニ選択", "Convenience", INT_LEN, 'n', array("EXIST_CHECK", "MAX_LENGTH_CHECK", "NUM_CHECK"));
    }
    ...
    function modeAction($mode, &$objFormParam, &$arrOrder, &$objPage) {
        ...
        case 'next':
            ...
            if(SC_Display_Ex::detectDevice() === DEVICE_TYPE_MOBILE
                    ...
                    && !SC_Utils_Ex::isBlank($objSess->GetSession('plg_pg2click_oldpaydata'))) {
                ...
                if (!SC_Utils_Ex::isBlank($arrOldParam['Convenience'])) {
                    $arrParam = array('Convenience' => $arrOldParam['Convenience']);
            ...
            if (SC_Utils_Ex::isBlank($objPage->arrErr)) {
                // 決済実行
                $objClient = new SC_Mdl_PG_MULPAY_Client_CVS_Ex();
        ...
        default:
            ...
            if ($objSess->GetSession('plg_pg2click_payment_confirm')) {
                ...
                $arrOldParam = $objSess->GetSession('plg_pg2click_oldpaydata');
                $objFormParam->setParam($arrOldParam);
                $objPage->tpl_onload .= "send = false; fnModeSubmit('next','','');";
    ...
    function getFormBloc() {
        ...
        $bloc_id =  $arrBlocId['pg_mulpay_cvs'][ $device_type_id ];
        if ($bloc_id) {
