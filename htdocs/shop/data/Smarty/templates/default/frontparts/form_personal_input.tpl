...
<!--{strip}-->
    ...
    <tr>
        ...
        <td>
            ...
            <!--{/if}-->
            <!--First name&nbsp;<input type="text" name="<!--{$key2}-->" value="<!--{if $arrForm[$key2].value|h}--><!--{$arrForm[$key2].value|h}--><!--{else}--><!--{php}-->echo $_SESSION[customer][name02];<!--{/php}--><!--{/if}-->" maxlength="<!--{$arrForm[$key2].length}-->" style="<!--{$arrErr[$key2]|sfGetErrorColor}-->; ime-mode: disabled;" class="box120" />-->
            <input type="text" name="<!--{$key1}-->" value="<!--{if $arrForm[$key1].value|h}--><!--{$arrForm[$key1].value|h}--><!--{else}--><!--{php}-->echo $_SESSION[customer][name01];<!--{/php}--><!--{/if}-->" maxlength="<!--{$arrForm[$key1].length}-->" style="<!--{$arrErr[$key1]|sfGetErrorColor}-->; ime-mode: disabled;" class="box300" />
            <p class="mini gray">Symbols ( , . = ' ･) are unable to use.<br>Please enter alphabetic characters only.</p>
        </td>
    </tr>
    <!--<tr>
    ...
    <!--{else}-->
     <input type="hidden" name="dummy<!--{$key6}-->" value="<!--{$smarty.const.DEFAULT_COUNTRY_ID}-->" />
    <!--<tr>
        <th>ZIP CODE</th>
        <td>
            〒<!--{if $arrErr[$key7]}-->
    ...
    <tr>
        <th>Address<span class="attention">*</span>
        <p><span class="attention mini gray">Please enter your home country address or your address in Japan.</span></p></th>
        <td>
            ...
            <p class="top"><input type="text" name="<!--{$key4}-->" value="<!--{if $arrForm[$key4].value|h}--><!--{$arrForm[$key4].value|h}--><!--{else}--><!--{php}-->echo $_SESSION[customer][addr01];<!--{/php}--><!--{/if}-->" class="box300" style="<!--{$arrErr[$key4]|sfGetErrorColor}-->; ime-mode: disabled;" /><br />
                Address1 <!--{$smarty.const.SAMPLE_ADDRESS1}--></p>
            <p class="top"><input type="text" name="<!--{$key5}-->" value="<!--{if $arrForm[$key5].value|h}--><!--{$arrForm[$key5].value|h}--><!--{else}--><!--{php}-->echo $_SESSION[customer][addr02];<!--{/php}--><!--{/if}-->" class="box300" style="<!--{$arrErr[$key5]|sfGetErrorColor}-->; ime-mode: disabled;" /><br />
    ...
    <tr>
        ...
        <td>
            ...
            <!--{/if}-->
            <input type="text" name="<!--{$key1}-->" value="<!--{if $arrForm[$key1].value|h}--><!--{$arrForm[$key1].value|h}--><!--{else}--><!--{php}-->echo $_SESSION[customer][tel01];<!--{/php}--><!--{/if}-->" maxlength="<!--{$arrForm[$key1].length}-->" style="<!--{$arrErr[$key1]|sfGetErrorColor}-->; ime-mode: disabled;" class="box300" />
        </td>
    </tr>
        ...
        <tr>
            ...
            <td>
                ...
                <!--{if $arrErr[$key1] || $arrErr[$key2]}-->
                    <div class="attention"><!--{$arrErr[$key1]}--><!--{$arrErr[$key2]}--></div>
                <!--{/if}-->
                <input type="text" name="<!--{$key1}-->" value="<!--{if $arrForm[$key1].value|h}--><!--{$arrForm[$key1].value|h}--><!--{else}--><!--{php}-->echo $_SESSION[customer][email];<!--{/php}--><!--{/if}-->" style="<!--{$arrErr[$key1]|sfGetErrorColor}-->; ime-mode: disabled;" class="box300 top" /><br />
                <input type="text" name="<!--{$key2}-->" value="<!--{if $arrForm[$key2].value|h}--><!--{$arrForm[$key2].value|h}--><!--{else}--><!--{php}-->echo $_SESSION[customer][email];<!--{/php}--><!--{/if}-->" style="<!--{$arrErr[$key1]|cat:$arrErr[$key2]|sfGetErrorColor}-->; ime-mode: disabled;" class="box300" /><br />
                <span class="mini gray">Re-enter email address</span>
            </td>
        </tr>
        <!--{if $emailMobile}-->
            <tr>
    ...
    <tr style="display: none">
        <th>[Those who added more than one router to the cart]<br />
            Would you like to use one unit continuously?</th>
        <td>
            <!--{assign var=key1 value="`$prefix`fax01"}-->
            <!--{assign var=key2 value="`$prefix`fax02"}-->
            <!--{assign var=key3 value="`$prefix`fax03"}-->
...
<!--			<option value="0" selected="selected">選択してください</option>-->
			...
			<input type="hidden" name="<!--{$key3}-->" value="1111" maxlength="<!--{$arrForm[$key3].length}-->" style="<!--{$arrErr[$key3]|sfGetErrorColor}-->; ime-mode: disabled;" class="box60" />
        </td>
    </tr>
        ...
        <td>
         ...
         <span class="mini">Front sticker number begin with FS, SL, SH, DF, etc.</span>
        </td>
			<!--{/if}-->
