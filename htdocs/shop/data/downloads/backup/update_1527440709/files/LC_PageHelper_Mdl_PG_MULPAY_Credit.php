...
class LC_PageHelper_Mdl_PG_MULPAY_Credit extends LC_PageHelper_Mdl_PG_MULPAY_Base_Ex {
    ...
    function initParam(&$objFormParam, &$arrPaymentInfo, &$arrOrder) {
        $objFormParam->addParam("Card number　", "CardNo", 16, 'n', array("EXIST_CHECK", "MAX_LENGTH_CHECK", "NUM_CHECK"));
        $objFormParam->addParam("Expiration date/year　", "Expire_year", 2, 'n', array("EXIST_CHECK", "MAX_LENGTH_CHECK", "NUM_CHECK"));
        $objFormParam->addParam("Expiration date/month　", "Expire_month", 2, 'n', array("EXIST_CHECK", "MAX_LENGTH_CHECK", "NUM_CHECK"));
        $objFormParam->addParam("Card holder's name　", "card_name1", 25, 'a', array("EXIST_CHECK", "MAX_LENGTH_CHECK", "ALNUM_CHECK"), "");
    ...
    function checkError(&$objFormParam) {
        ...
        if (SC_Utils_Ex::isBlank($objErr->arrErr)) {
            if (strlen($arrParam['CardNo']) < 10 || strlen($arrParam['CardNo']) > 16) {
                $objErr->arrErr['CardNo'] = '※ The digit of card number is lacking<br />';
            }
        ...
        case 'next':
            ...
            if (SC_Utils_Ex::isBlank($objPage->arrErr)) {
                ...
                if ($result) {
                    ...
                    $objPage->arrErr['payment'] = '※ Error occurred in payment<br />' . implode('<br />', $arrErr);
                }
            } else {
