...
<section class="pay_area02">
    <h3 class="subtitle">営業店・郵便局留めをご希望の場合</h3>
    ...
    <div class="delivdate top">
        ...
        <span class="attention"><!--{$arrErr[$key]}--></span>
        営業店コード・郵便局名を入力　<input type="text" name="<!--{$key}-->" class="boxLong data-role-none" value="<!--{$arrForm[$key].value|default:$shippingItem.plg_expresslink_center_code|h}-->" style="<!--{$arrErr[$key]|sfGetErrorColor}-->"><!--{if $smarty.const.plg_ExpressLink_Use_YuPack4 == 1 || $smarty.const.plg_ExpressLink_Use_YuPackR == 1}--><br>
        <!--{assign var=key value="plg_expresslink_center_zip`$index`"}-->
        <span class="attention"><!--{$arrErr[$key]}--></span>
        局留め郵便番号　<input type="text" name="<!--{$key}-->" class="boxLong data-role-none" value="<!--{$arrForm[$key].value|default:$shippingItem.plg_expresslink_center_zip|h}-->" style="<!--{$arrErr[$key]|sfGetErrorColor}-->"><br>
