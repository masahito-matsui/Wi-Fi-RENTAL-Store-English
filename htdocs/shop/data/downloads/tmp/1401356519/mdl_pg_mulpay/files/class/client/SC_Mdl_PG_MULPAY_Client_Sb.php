 ...
 */
class SC_Mdl_PG_MULPAY_Client_Sb extends SC_Mdl_PG_MULPAY_Client_Base_Ex {
    ...
    function doSbReturn($arrOrder, $arrParam, $arrPaymentInfo) {

        if (SC_Utils_Ex::isBlank($arrOrder[MDL_PG_MULPAY_ORDER_COL_PAYDATA])) {
            $msg = MDL_PG_MULPAY_PAYNAME_SB . '決済遷移エラー:決済データが受注情報に見つかりませんでした.';
            $objMdl->printLog($msg);
            SC_Utils_Ex::sfDispSiteError(FREE_ERROR_MSG, "", true,
                "例外エラー<br />" . MDL_PG_MULPAY_PAYNAME_SB . "遷移エラー<br />この手続きは無効となりました。<br />決済データが受注情報に見つかりませんでした。");
        ...
        if ($arrParam['Status'] == 'PAYFAIL') {
            $msg = MDL_PG_MULPAY_PAYNAME_SB . '決済に失敗しました。';
            $this->setError($msg);
        }

        if (SC_Utils_Ex::isBlank($arrParam['SbTrackingId'])) {
            $msg = MDL_PG_MULPAY_PAYNAME_SB . 'がキャンセルされました。';
            $this->setError($msg);
        }
    ...
    function doPaymentRequest($arrOrder, $arrParam, $arrPaymentInfo) {
        ...
        $server_url = $arrMdlSetting['server_url'] . 'EntryTranSb.idPass';
        ...
        $server_url = $arrMdlSetting['server_url'] . 'ExecTranSb.idPass';
        $arrSendKey = array(
            'ShopID',
            'ShopPass',
            'AccessID',
