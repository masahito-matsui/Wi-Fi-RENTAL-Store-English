...
/**
 * 決済モジュール 決済処理: WebMoney
 */
class SC_Mdl_PG_MULPAY_Client_WebMoney extends SC_Mdl_PG_MULPAY_Client_Base_Ex {
    ...
    function getWebmoneyStartUrl() {
        $objMdl =& SC_Mdl_PG_MULPAY_Ex::getInstance();
        $arrMdlSetting = $objMdl->getUserSettings();

        $server_url = $arrMdlSetting['server_url'] . 'WebmoneyStart.idPass';
        return $server_url;
    }

    function doWebMoneyReturn($arrOrder, $arrParam, $arrPaymentInfo) {
        $this->setResults($arrParam);

        if (SC_Utils_Ex::isBlank($arrOrder[MDL_PG_MULPAY_ORDER_COL_PAYDATA])) {
            $msg = 'WebMoney遷移エラー:決済データが受注情報に見つかりませんでした.';
            $objMdl->printLog($msg);
            SC_Utils_Ex::sfDispSiteError(FREE_ERROR_MSG, "", true,
                "例外エラー<br />WebMoney遷移エラー<br />この手続きは無効となりました。<br />決済データが受注情報に見つかりませんでした。");
            SC_Response_Ex::actionExit();
        } else {
            $arrPayData = unserialize($arrOrder[MDL_PG_MULPAY_ORDER_COL_PAYDATA]);
        }

        if ($arrParam['PayCancel'] == '1') {
            $msg = 'WebMoenyの支払い操作がキャンセルされました。';
            $this->setError($msg);
        }
        ...
        if (SC_Utils_Ex::isBlank($this->getError())) {
            if ($arrParam['Status'] == 'PAYSUCCESS') {
                $arrParam['pay_status'] = MDL_PG_MULPAY_PAY_STATUS_PAY_SUCCESS;
            } else if ($arrParam['Status'] == 'REQSUCCESS') {
                $arrParam['pay_status'] = MDL_PG_MULPAY_PAY_STATUS_REQUEST_SUCCESS;
            }
        }else {
            if ($arrParam['PayCancel'] == '1') {
                $arrParam['pay_status'] = MDL_PG_MULPAY_ACTION_STATUS_CANCEL;
            } else {
...
/*
        if(!isset($arrOrder['PgTransactionId'])) {
            $arrOrder['PgTransactionId'] = SC_Utils_Ex::sfGetRandomString(8);
            SC_Util_PG_MULPAY_Ex::setOrderPayData($arrOrder, array('PgTransactionId' => $arrOrder['PgTransactionId']));
        }
*/
        $server_url = $arrMdlSetting['server_url'] . 'EntryTranWebmoney.idPass';
        ...
        $server_url = $arrMdlSetting['server_url'] . 'ExecTranWebmoney.idPass';
        $arrSendKey = array(
            'ShopID',
            'ShopPass',
            'AccessID',
