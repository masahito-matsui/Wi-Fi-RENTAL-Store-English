    ...
    function getSendData($arrSendKey, $arrOrder, $arrParam, $arrPaymentInfo, $arrMdlSetting) {
        ...
        foreach ($arrSendKey as $key) {
            ...
            case 'Commodity':
                ...
                break;
            case 'Token':
                $arrSendData[$key] = $arrParam['token'];
                break;
            case 'RetURL':
            case 'RedirectURL':
...
//                    $arrSendData[$key] .= '&' . session_name() . '=' . session_id();
    ...
    function convOld2NewCVSCD($data) {
        if (isset($data['Convenience']) &&
            ...
            case CONVENI_LOSON:
                $data['Convenience'] = CONVENI_LOSON_NEW;
                break;

            case CONVENI_FAMILYMART:
                $data['Convenience'] = CONVENI_FAMILYMART_NEW;
                break;

            case CONVENI_SUNKUS:
                $data['Convenience'] = CONVENI_SUNKUS_NEW;
                break;

            case CONVENI_CIRCLEK:
                $data['Convenience'] = CONVENI_CIRCLEK_NEW;
                break;

            case CONVENI_MINISTOP:
                $data['Convenience'] = CONVENI_MINISTOP_NEW;
                break;

            case CONVENI_SEICOMART:
                $data['Convenience'] = CONVENI_SEICOMART_NEW;
                break;
        ...
        for ($i = 0; $i < count($data); ++$i) {
            if (isset($data[$i]['Convenience']) &&
                ...
                case CONVENI_LOSON_NEW:
                    $data[$i]['Convenience'] = CONVENI_LOSON;
                    break;

                case CONVENI_FAMILYMART_NEW:
                    $data[$i]['Convenience'] = CONVENI_FAMILYMART;
                    break;

                case CONVENI_SUNKUS_NEW:
                    $data[$i]['Convenience'] = CONVENI_SUNKUS;
                    break;

                case CONVENI_CIRCLEK_NEW:
                    $data[$i]['Convenience'] = CONVENI_CIRCLEK;
                    break;

                case CONVENI_MINISTOP_NEW:
                    $data[$i]['Convenience'] = CONVENI_MINISTOP;
                    break;

                case CONVENI_SEICOMART_NEW:
                    $data[$i]['Convenience'] = CONVENI_SEICOMART;
                    break;
    ...
    function sendRequest($url, $arrSendData) {
        // コンビニコードの新旧コード変換を行う
        $arrSendData = $this->convOld2NewCVSCD($arrSendData);
    ...
    function setResults($arrResults) {
        ...
        // コンビニコードの新旧コード変換を行う
        $arrResults = $this->convNew2OldCVSCD($arrResults);
    ...
    function setOrderPaymentViewData($arrOrder, $arrParam, $arrPaymentInfo) {
        ...
        if (!SC_Utils_Ex::isBlank($arrOrder['Convenience'])) {
            if ($arrOrder['Convenience'] == CONVENI_LOSON ||
                $arrOrder['Convenience'] == CONVENI_MINISTOP) {
                if (!SC_Utils_Ex::isBlank($arrOrder['ReceiptNo'])) {
                    $arrData['ReceiptNo']['name'] = 'お客様番号';
                    $arrData['ReceiptNo']['value'] = $arrOrder['ReceiptNo'];
                }
            } else if ($arrOrder['Convenience'] == CONVENI_CIRCLEK ||
                       $arrOrder['Convenience'] == CONVENI_SUNKUS ||
                       $arrOrder['Convenience'] == CONVENI_SEICOMART) {
                if (!SC_Utils_Ex::isBlank($arrOrder['ReceiptNo'])) {
        ...
        if (!SC_Utils_Ex::isBlank($arrOrder['Convenience']) &&
            $arrOrder['Convenience'] == CONVENI_FAMILYMART) {
            if (!SC_Utils_Ex::isBlank($arrOrder['ConfNo'])) {
                $arrData['ConfNo']['name'] = '企業コード';
        ...
        if (!SC_Utils_Ex::isBlank($arrOrder['Convenience'])) {
            if ($arrOrder['Convenience'] == CONVENI_FAMILYMART) {
                if (!SC_Utils_Ex::isBlank($arrOrder['ReceiptNo'])) {
                    $arrData['ReceiptNo']['name'] = '注文番号';
                    $arrData['ReceiptNo']['value'] = $arrOrder['ReceiptNo'];
                }
            } else if ($arrOrder['Convenience'] == CONVENI_DAILYYAMAZAKI ||
                       $arrOrder['Convenience'] == CONVENI_SEVENELEVEN ||
                       $arrOrder['Convenience'] == CONVENI_THREEF) {
                if (!SC_Utils_Ex::isBlank($arrOrder['ReceiptNo'])) {
                    $arrData['ReceiptNo']['name'] = '受付番号';
                    $arrData['ReceiptNo']['value'] = $arrOrder['ReceiptNo'];
                }
        ...
        if (!SC_Utils_Ex::isBlank($arrOrder['ReceiptUrl'])) {
            $arrData['ReceiptUrl']['name'] = '払込票URL';
            $arrData['ReceiptUrl']['value'] = $arrOrder['ReceiptUrl'];
        }
        ...
        if (!SC_Utils_Ex::isBlank($arrOrder['Convenience'])) {
            $key = $arrOrder['Convenience'];
            $title_key = 'order_mail_title_' . $key;
            $arrData[$title_key]['name'] = $this->lfGetCvsGuidanceTitle($key);
            $arrData[$title_key]['value']  = "\n";
            $arrData[$title_key]['value'] .= $this->lfGetCvsGuidanceBody($key);
        }
    ...
    function lfGetCvsGuidanceTitle($cvs_id) {
        ...
        if (SC_Utils_Ex::isBlank($arrCONVENI[$cvs_id])) {
            return "";
        }
