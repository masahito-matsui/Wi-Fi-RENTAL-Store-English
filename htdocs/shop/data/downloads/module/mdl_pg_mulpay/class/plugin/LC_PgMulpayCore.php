...
class LC_PgMulpayCore {
    ...
    function actionHook($class_name, $hook_point, &$objPage, $objPlugin) {
        ...
        case 'LC_Page_Admin_Basis_PaymentInput_Ex':
            if ($hook_point == 'after') {
                ...
                $objPage->arrAllowFlags = array('0' => '利用する', '1' => '利用しない');
    ...
    function lfCheckErrorPaymentInput(&$objFormParam, $pay_id) {
        ...
        if (!SC_Utils_Ex::isBlank($arrRet['use_limit']) && $arrRet['use_limit'] == "1") {
            $objErr->doFunc(array('入力回数制限 検出時間', 'limit_min'), array('EXIST_CHECK'));
            $objErr->doFunc(array('入力回数制限 エラー上限回数', 'limit_count'), array('EXIST_CHECK'));
            $objErr->doFunc(array('入力回数制限 ロック時間', 'lock_min'), array('EXIST_CHECK'));
        }
...
';
    ...
    function lfInitParamPaymentInput(&$objFormParam, $pay_id, &$arrPayConfig) {
        ...
        if ($pay_id == MDL_PG_MULPAY_PAYID_CREDIT || $pay_id == MDL_PG_MULPAY_PAYID_REGIST_CREDIT
            ...
            || $pay_id == MDL_PG_MULPAY_PAYID_TOKEN
        ...
        $key = 'use_limit';
        $objFormParam->addParam("入力回数制限", $key, 1, "a",
             array("NUM_CHECK", "MAX_LENGTH_CHECK"),
             SC_Utils_Ex::isBlank($arrPayConfig[$key]) ? '0' : $arrPayConfig[$key]
             );

        $key = 'limit_min';
        $objFormParam->addParam("入力回数制限 検出時間", $key, 3, "a",
             array("MAX_LENGTH_CHECK","NUM_CHECK"),
             SC_Utils_Ex::isBlank($arrPayConfig[$key]) ? '' : $arrPayConfig[$key]
             );

        $key = 'limit_count';
        $objFormParam->addParam("入力回数制限 エラー上限回数", $key, 2, "a",
             array("MAX_LENGTH_CHECK","NUM_CHECK"),
             SC_Utils_Ex::isBlank($arrPayConfig[$key]) ? '' : $arrPayConfig[$key]
             );

        $key = 'lock_min';
        $objFormParam->addParam("入力回数制限 ロック時間", $key, 3, "a",
             array("MAX_LENGTH_CHECK","NUM_CHECK"),
             SC_Utils_Ex::isBlank($arrPayConfig[$key]) ? '' : $arrPayConfig[$key]
             );
        ...
        if (false && $pay_id == MDL_PG_MULPAY_PAYID_CVS) {
