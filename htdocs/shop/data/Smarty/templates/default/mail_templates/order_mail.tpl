...
<!--{assign var="kikaku1" value="-"|explode:$arrOrderDetail[cnt].classcategory_name1}-->
<!--{$kikaku1[1]}-->
<!--{if $arrOrder.deliv_id == 5}-->
Extension rental period: <!--{$kikaku1[0]+1}--> days
Are you renting an additional battery?: <!--{$arrOrderDetail[cnt].classcategory_name2}-->
<!--{elseif $arrOrder.deliv_id == 8}-->
<!--{elseif $arrOrder.deliv_id == 9}-->
<!--{else}-->
Rental period: <!--{$kikaku1[0]+1}--> days
Options: <!--{$arrOrderDetail[cnt].classcategory_name2}-->
<!--{/if}-->
...
Subtotal: <!--{$arrOrder.subtotal-$total_inctax|number_format|default:0}--> JPY
<!--{php}-->} else {<!--{/php}-->
...
<!--{else}-->
  <!--{if $arrOrder.deliv_id == 9}-->
		<!--{else}-->
  Rental Start Date: <!--{$arrOrder.start_date|date_format:"%Y/%m/%d"|default:"Not specified"}-->
  Rental End Date: <!--{$arrOrder.end_date|date_format:"%Y/%m/%d"|default:"Not specified"}-->
  <!--{/if}-->
...
□下記の「ご利用開始URL」から、手順に従ってご利用を開始してください。
  ...
  ご利用開始URL：<!--{$arreSIM[cnt].url}-->
　eSIM番号：<!--{$arreSIM[cnt].iccid}-->
<!--{/if}-->
<!--{/section}-->
<!--{* update for eSIM *}-->
