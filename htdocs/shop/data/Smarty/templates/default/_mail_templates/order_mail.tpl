...
(*To the customers for extension)
The confirmation email will be sent when the extension process is completed manually.
The renewed expiring date will be informed in the message.
Please keep using the current device continuously. 
<!--{else}-->
The tracking number of the parcel will be informed by email when the shipment is completed.
<!--{/if}-->
...
<!--{else}-->
<!--{if false}-->Commodity code: <!--{$arrOrderDetail[cnt].product_code}--><!--{/if}-->
<!--{if $arrOrder.deliv_id == 5}-->
Name of commodity： <!--{$arrOrderDetail[cnt].product_name}-->
Are you renting an additional battery?：<!--{$arrOrderDetail[cnt].classcategory_name1}-->
...
Subtotal ￥<!--{$arrOrder.subtotal-1080|number_format|default:0}--> <!--{if 0 < $arrOrder.tax}-->(Consumption tax ￥<!--{$arrOrder.tax-80|number_format|default:0}-->)<!--{/if}-->
<!--{php}-->} else {<!--{/php}-->
...
Reduced fee ￥<!--{$arrOrder.discount|number_format|default:0}-->
<!--{/if}-->
<!--{if $arrOrder.use_point > 0}-->
Point discout ￥<!--{$arrOrder.use_point*$smarty.const.POINT_VALUE}-->
<!--{/if}-->
Delivery fee ￥<!--{$arrOrder.deliv_fee|number_format|default:0}-->
<!--{php}-->if($add_charge_flg == 1) {<!--{/php}-->
Additional delivery fee ￥1,080
<!--{php}-->}<!--{/php}-->
<!--{if $arrOrder.charge > 0}-->
Modified amount ￥<!--{$arrOrder.charge|number_format|default:0}-->
<!--{/if}-->
============================================
...
<!--{elseif $arrOrder.deliv_id == 5}-->
　Start date for extension：<!--{$shipping.shipping_date|date_format:"%Y/%m/%d"|default:"指定なし"}-->
