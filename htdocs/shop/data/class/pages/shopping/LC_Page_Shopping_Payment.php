...
class LC_Page_Shopping_Payment extends LC_Page_Ex
{
    ...
    public function init()
    {
        ...
        $this->tpl_title = 'Delivery Date/Time';
...
//                        $url = DELIV_URLPATH;
    ...
    public function lfInitParam(&$objFormParam, $deliv_only, &$arrShipping)
    {
        ...
        if ($deliv_only) {
            $objFormParam->addParam('Payment method', 'payment_id', INT_LEN, 'n', array('MAX_LENGTH_CHECK', 'NUM_CHECK'));
        } else {
            $objFormParam->addParam('Payment method', 'payment_id', INT_LEN, 'n', array('EXIST_CHECK', 'MAX_LENGTH_CHECK', 'NUM_CHECK'));

            foreach ($arrShipping as $val) {
                $objFormParam->addParam('Delivery time', 'deliv_time_id' . $val['shipping_id'], INT_LEN, 'n', array('MAX_LENGTH_CHECK', 'NUM_CHECK'));
                $objFormParam->addParam('Delivery Date ', 'deliv_date' . $val['shipping_id'], STEXT_LEN, 'KVa', array('EXIST_CHECK', 'MAX_LENGTH_CHECK'));
            }
        }
    ...
    public function lfCheckError(&$objFormParam, $subtotal, $max_point)
    {
        ...
        if ($arrForm['point_check'] == '1') {
            $objErr->doFunc(array('Use points', 'point_check'), array('EXIST_CHECK'));
            $objErr->doFunc(array('Point', 'use_point'), array('EXIST_CHECK'));
            if ($max_point == '') {
                $max_point = 0;
            }
            // FIXME mobile 互換のため br は閉じない...
            if ($arrForm['use_point'] > $max_point) {
                $objErr->arrErr['use_point'] = '※used points exceeds available points<br>';
            }
            if (($arrForm['use_point'] * POINT_VALUE) > $subtotal) {
                $objErr->arrErr['use_point'] = '※Used points exceeds purchased amount<br>';
            }
            // ポイント差し引き後の決済方法チェック
