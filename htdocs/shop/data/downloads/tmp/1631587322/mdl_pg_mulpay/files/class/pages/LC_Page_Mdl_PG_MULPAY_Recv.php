...
//        $this->sendResponse();
    ...
    function action() {
        ...
        if (SC_Utils_Ex::isBlank($arrErr)) {
            ...
            if (strcmp($status, 'REQSUCCESS') == 0 ||
                strcmp($status, 'AUTHPROCESS') == 0) {
                $objMdl =& SC_Mdl_PG_MULPAY_Ex::getInstance();
                $objMdl->printLog(sprintf('Receive %s normal exit.', $status));
                $this->lfSendResponse(true);
                SC_Response_Ex::actionExit();
            } else if (strcmp($status, 'AUTH') == 0 ||
                       strcmp($status, 'CHECK') == 0 ||
                       strcmp($status, 'CAPTURE') == 0) {
                sleep(MDL_PG_MULPAY_RECEIVE_WAIT_TIME);
            }
    ...
    function lfDoReceive(&$arrParam, &$arrOrder) {
        ...
        default:
            if ($arrOrder[MDL_PG_MULPAY_ORDER_COL_PAYID] != MDL_PG_MULPAY_PAYID_CREDIT
                ...
                && $arrOrder[MDL_PG_MULPAY_ORDER_COL_PAYID] != MDL_PG_MULPAY_PAYID_TOKEN
                 ) {
                $this->lfDoUnMatchPayType($arrParam, $arrOrder);
                $res = false;
            } else {
