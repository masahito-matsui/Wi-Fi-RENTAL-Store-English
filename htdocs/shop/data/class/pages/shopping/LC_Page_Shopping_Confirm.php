...
class LC_Page_Shopping_Confirm extends LC_Page_Ex
{
    ...
    {
        parent::init();
        $this->tpl_title = 'Confirmation of entry';
...
// update for eSIM
        // echo "<pre>"; print_r($this->arrCartItems); exit;
        $quantity = 0;
        $product_code;
        foreach ($this->arrCartItems as $key => $value) {
          $quantity = $quantity + intval($value['quantity']);
          $product_code = $value['productsClass']['product_code'];
          $maker_id = $value['productsClass']['maker_id'];
        }

        if($maker_id == 3) {
          if($quantity > 1) {
            //カート内の商品が1つでないのでカートへ戻す
            SC_Response_Ex::sendRedirect(CART_URL);
            SC_Response_Ex::actionExit();
          }
          ...
          // print_r(count($arrRes)); exit;
          if(count($arrRes) < 1) {
            //dtb_esim_stockのproduct_codeで登録可能数がないのでカートへ戻す（※処理変更は必要そう）
            SC_Response_Ex::sendRedirect(CART_URL);
            SC_Response_Ex::actionExit();
          }
