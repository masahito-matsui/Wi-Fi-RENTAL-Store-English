...
require_once(MDL_PG_MULPAY_CLASSEX_PATH . 'client_extends/SC_Mdl_PG_MULPAY_Client_PayEasy_Ex.php');

/**
 * 決済モジュール 決済画面ヘルパー：pay-easy決済
 */
class LC_PageHelper_Mdl_PG_MULPAY_PayEasy extends LC_PageHelper_Mdl_PG_MULPAY_Base_Ex {
    ...
    function modeAction($mode, &$objFormParam, &$arrOrder, &$objPage) {
        ...
        case 'next':
            ...
            if (SC_Utils_Ex::isBlank($objPage->arrErr)) {
                // 決済実行
                $objClient = new SC_Mdl_PG_MULPAY_Client_PayEasy_Ex();
                ...
                if ($result) {
                    ...
                    $objPage->EncryptReceiptNo = $arrResults['EncryptReceiptNo'];
                    if (SC_Display_Ex::detectDevice() === DEVICE_TYPE_MOBILE) {
                        $objPage->tpl_url = $objPage->arrPaymentInfo['SelectPageCall_Mobile'];
                    } else {
                        $objPage->tpl_url = $objPage->arrPaymentInfo['SelectPageCall_PC'];
                    }
                    // 設定されていない場合は完了画面へ
                    if ($objPage->tpl_url == "") {
                    ...
                    $objPage->tpl_payment_onload = 'setTimeout("fnAutoLoadSubmit()", 20000);';
                } else {
    ...
    function getFormBloc() {
        ...
        $bloc_id =  $arrBlocId['pg_mulpay_payeasy'][ $device_type_id ];
        if ($bloc_id) {
