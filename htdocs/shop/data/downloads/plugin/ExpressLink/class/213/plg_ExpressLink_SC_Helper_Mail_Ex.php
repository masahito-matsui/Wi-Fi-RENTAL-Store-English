...
require_once PLUGIN_UPLOAD_REALDIR . "ExpressLink/class/plg_ExpressLink_SC_Helper_Mail.php";

class plg_ExpressLink_SC_Helper_Mail_Ex extends plg_ExpressLink_SC_Helper_Mail
{
    /* 受注完了メール送信 */
    ...
    function sfSendOrderMail($order_id, $template_id, $subject = '', $header = '', $footer = '', $send = true)
    {
        ...
        $arrTplVar->arrDeliv = SC_Helper_Delivery_Ex::getIDValueList();
        ...
        // update for eSIM
        $where = 'order_id = ?';
        $arrTplVar->arreSIM = $objQuery->select('*', 'dtb_esim_stock', $where, array($order_id));
        // update for eSIM
        ...
        $arrRet = $this->sfGetShippingData($order_id);
    ...
    function sfGetShippingData($order_id)
    {
        ...
        foreach ($arrRet as $key => $value) {
            ...
            if(version_compare(ECCUBE_VERSION,'2.13.2','>=')){
                $arrRet[$key]['shipment_item'] = $arrItems;
            }else{
                foreach ($arrItems as $key => $arrDetail) {
