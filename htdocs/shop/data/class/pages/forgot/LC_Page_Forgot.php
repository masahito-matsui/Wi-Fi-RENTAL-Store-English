...
*/
        $where = '(email = ? OR email_mobile = ?) AND name01 = ? AND del_flg = 0';
        $arrVal = array($arrForm['email'], $arrForm['email'], $arrForm['name01']);
        ...
        if (isset($result[0]['reminder']) and isset($arrReminder[$result[0]['reminder']])) {
            ...
            $errmsg = 'Your name or email address is invalid';
        }
    ...
    {
        $objFormParam->addParam('Name', 'name01', STEXT_LEN, 'aKV', array('EXIST_CHECK', 'SPTAB_CHECK' ,'MAX_LENGTH_CHECK'));
        $objFormParam->addParam('Name(名)', 'name02', STEXT_LEN, 'aKV', array('NO_SPTAB', 'SPTAB_CHECK' , 'MAX_LENGTH_CHECK'));
        if ($device_type === DEVICE_TYPE_MOBILE) {
            $objFormParam->addParam('Email', 'email', null, 'a', array('EXIST_CHECK', 'EMAIL_CHECK', 'NO_SPTAB' ,'EMAIL_CHAR_CHECK', 'MOBILE_EMAIL_CHECK'));
        } else {
            $objFormParam->addParam('Email', 'email', null, 'a', array('NO_SPTAB', 'EXIST_CHECK', 'EMAIL_CHECK', 'SPTAB_CHECK' ,'EMAIL_CHAR_CHECK'));
        }
...
//                    . ' AND name01 = ? AND name02 = ?'
                    . ' AND concat(ifnull(name01, \'\'), ifnull(name02, \'\')) = ?'
...
//                            $arrForm['name01'], $arrForm['name02']);
    ...
    public function lfInitSecretCheckParam(&$objFormParam, $device_type)
    {
        ...
        // 秘密の質問チェックの追加
        $objFormParam->addParam('Question for confirmation of password', 'reminder', STEXT_LEN, 'n', array('EXIST_CHECK', 'NUM_CHECK'));
        $objFormParam->addParam('Answer ', 'reminder_answer', STEXT_LEN, 'aKV', array('EXIST_CHECK', 'SPTAB_CHECK', 'MAX_LENGTH_CHECK'));
    ...
    public function lfSendMail(&$CONF, $email, $customer_name, $new_password)
    {
        ...
        $objMail->setItem(
            '' //宛先
            , $objHelperMail->sfMakeSubject('Password has been changed')
            , $toCustomerMail //本文
