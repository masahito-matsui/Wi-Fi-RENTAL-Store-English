...
class SC_Mdl_PG_MULPAY {
	...
	// 接続先設定の入力指定を廃止する
	// 入力指定が選択されている場合は本番環境に変更する
	$userSettings = $this->getUserSettings();
	if (!is_null($userSettings) &&
	    isset($userSettings['connect_server_type']) &&
	    $userSettings['connect_server_type'] == '3') {	// 入力指定
	    $userSettings['connect_server_type'] = '2';		// 本番環境
	    $userSettings['server_url'] = MDL_PG_MULPAY_SERVER_URL_PROD;
	    $userSettings['kanri_server_url'] = MDL_PG_MULPAY_KANRI_URL_PROD;

            $this->registerSubdata($userSettings, 'user_settings');
	}
    }
